  ...
  describe '#application_email' do
    it 'should send to candidate email' do
      ...
      candidate = Candidate.create!(email: 'candidate@test.com',
                                    password: '123456')
      other_candidate = Candidate.create!(email: 'candidate2@test.com',
                                          password: '123456')
      Profile.create!(candidate: candidate,
                      name: 'Gustavo', last_name: 'Carvalho',
                      social_name: 'Gustavo',
                      birthday: '20/01/1994',
    ...
    it 'should have a subject' do
      ...
      expect(mail.subject).to include("Caro #{candidate.profile.name}, " \
    ...
    it 'should have a body' do
      ...
      expect(mail.body).to include("Caro #{application.candidate.profile.name},"\
          " sua candidatura para #{application.job.title}" \
          ' foi realizada com sucesso!')
    end
  end

  describe "#cancelation_email" do
    it 'should send email to candidate email' do
      headhunter = Headhunter.create!(email: 'test@test.com',
                                      password: '123456')
      job = Job.create!(title: 'Programador RoR',
                        level: 'Júnior',
                        number_of_vacancies: 4,
      ...
      mail = JobApplicationMailer.cancelation_email(application.id)

      expect(mail.to).to include(candidate.email)
      expect(mail.to).not_to include(other_candidate.email)
    end

    it 'shoud came from a default email' do
      headhunter = Headhunter.create!(email: 'test@test.com',
                                      password: '123456')
      job = Job.create!(title: 'Programador RoR',
                        level: 'Júnior',
    ...
    it 'should have a subject' do
      ...
      expect(mail.subject).to eq('Inscrição para Programador RoR cancelada.')
    end
